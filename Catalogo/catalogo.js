...
// ============================
let cakes = []; // se llena al cargar la página

async function cargarTortasDesdeSupabase() {
  ...
  if (error) {
    ...
    grid.innerHTML = '<div class="col-12 text-center py-5"><p class="text-muted">Hubo un error al cargar el catálogo. Intenta de nuevo más tarde.</p></div>';
    return;
  }

  // Mapeamos los nombres de columnas de Supabase a los nombres que usa tu código (name, description, price, image, style)
  cakes = data.map(t => ({
    id: t.id, // uuid (string), no number
    name: t.nombre,
    description: t.descripcion,
    price: t.precio_base,
    style: t.categoria, // usamos categoria como "estilo" por ahora
    image: t.imagen,
...
const filterSize = document.getElementById('filter-tamano'); // ya no se usa para filtrar, pero lo dejamos por si existe en el HTML
        ...
        col.innerHTML = `
            <div class="card h-100 cake-card">
                ...
                <div class="card-body d-flex flex-column text-center p-4">
                    ...
                    <div class="price-tag mb-3">S/ ${Number(cake.price).toFixed(2)}</div>
...
function applyFilters() {
    ...
    // Filtro de tamaño desactivado temporalmente (no existe esa columna en Supabase aún)
    if (priceVal !== 'all') {
        filtered = filtered.filter(c => {
...
function renderSuggestions(query) {
    ...
    suggestionsEl.innerHTML = results.map(c => `
        <div class="suggestion-item" data-id="${c.id}">
            ...
            <span class="suggestion-price">S/ ${Number(c.price).toFixed(2)}</span>
        </div>
    `).join('');
...
function selectSuggestion(id) {
    const cake = cakes.find(c => c.id === id); // id es uuid (string), ya no parseInt
    if (!cake) return;
...
if (filterSize) filterSize.addEventListener('change', applyFilters); // no afecta el filtrado, pero no rompe si el elemento existe
...
searchInput.addEventListener('keydown', function (e) {
    ...
    if (e.key === 'Enter' && suggestionsVisible) {
        ...
        if (first) {
            selectSuggestion(first.dataset.id); // ya no parseInt, es uuid string
        }
    }
});
...
suggestionsEl.addEventListener('click', function (e) {
    ...
    if (item) {
        selectSuggestion(item.dataset.id); // ya no parseInt
    }
});
...
document.addEventListener('DOMContentLoaded', () => cargarTortasDesdeSupabase());