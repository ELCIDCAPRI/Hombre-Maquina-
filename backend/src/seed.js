...
async function seed() {
  try {
    ...
    await pool.query(
      ...
      ['Administrador', 'admin@tallerdesabores.pe', hashedPassword, 'admin', 'admin@tallerdesabores.pe']
    );
    ...
    for (const [nombre, descripcion, precio, estilo, tamaño, imagen_url, categoria_id] of pasteles) {
      await pool.query(
        'INSERT IGNORE INTO pasteles (nombre, descripcion, precio, estilo, tamaño, imagen_url, categoria_id) VALUES (?, ?, ?, ?, ?, ?, ?)',
        [nombre, descripcion, precio, estilo, tamaño, imagen_url, categoria_id]
      );
    }
